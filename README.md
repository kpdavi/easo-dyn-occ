...
* **data** &#x1F4C1;. This folder contains the raw (**raw** &#x1F4C1;) and processed (**processed** &#x1F4C1;) data for the Eastern screech-owl surveys and environmental covariates.
  + **raw** &#x1F4C1;
    - `RAP_tree_cover_2013-2021.tif`🖼. An image of tree cover rasters from the [Rangeland Analysis Platform](https://rangelands.app/rap/?biomass_t=herbaceous&ll=39.0000,-98.0000&z=5) from 2013--2021. This image is comprised of 9 remotely-sensed layers of continuous tree cover (one layer for each year) at a 30-m^2^ spatial resolution (Allred et al. 2021).
    - `sites.csv`![](https://upload.wikimedia.org/wikipedia/commons/3/35/CSV_Icon.png). The site IDs (*Transect*) and randomly-generated coordinates (*longitude* and *latitude*). The locations of the monitoring sites are not public, so coordinates were randomly generated so that the script would run. However, because the coordinates are not the actual coordinates for the sites, the values in the output file created in this script will not align with the data values used in the manuscript. Data with the real covariate values for tree cover are in `covs_tree.csv` in  **processed** &#x1F4C1;.
    - `surveys.csv`![](https://upload.wikimedia.org/wikipedia/commons/3/35/CSV_Icon.png). The Eastern screech-owl survey data. Columns include site ID (*Transect*), year (*year*; 2013--2021), survey date (*date*), survey start time (*start.time*), presence / absence of the species (*presence*; 0 [absence] or 1 [presence]).
  + **processed** &#x1F4C1;
    - `covs_climate.csv`![](https://upload.wikimedia.org/wikipedia/commons/3/35/CSV_Icon.png). The climate covariate data. Columns include year (*year*; 2013--2021), average breeding season minimum temperature (&deg;C) in the current year (*avg.breeding.tmin*) and the breeding season prior (*lag.avg.breeding.tmin*), average winter minimum temperature (&deg;C) preceding the breeding season (*avg.winter.tmin*) and the winter a year prior (*lag.avg.winter.tmin*), and cumulative winter precipitation (mm) preceding the breeding season (*sum.winter.prcp*).
    - `covs_lidar.csv`![](https://upload.wikimedia.org/wikipedia/commons/3/35/CSV_Icon.png). The data for standard deviation in vegetation height in 2013. Columns include site ID (*Transect*), year (*year*; 2013), and standard deviation in vegetation height within 250-m-radii buffers surrounding the centroid of each site (meters; *sd.vegheight.m.2013*).
    - `covs_tree.csv`![](https://upload.wikimedia.org/wikipedia/commons/3/35/CSV_Icon.png). The data for proportion and cohesion of tree cover. Columns include site ID (*Transect*), year (*year*; 2013--2021), proportion of tree cover (was multiplied by 100 so values are percentages; *tree.prop*), and cohesion of tree cover (%; *tree.coh*).
    - `model_data.RData`![](https://upload.wikimedia.org/wikipedia/commons/3/35/CSV_Icon.png). Dataframes, arrays, and vectors needed to fit and / or plot results from the dynamic occupancy model.
      - `coeff_names`. A dataframe that includes the column names of covariates (*names_cov*), the real name of the covariate (*names_real*), the associated parameter in the dynamic occupancy model (*param*), and the process the covariate is fit on (*proc*).
    ...
    - `surveys_covs_dat.csv`![](https://upload.wikimedia.org/wikipedia/commons/3/35/CSV_Icon.png). The Eastern screech-owl survey data and data for detection covariates. Columns include site ID (*Transect*), year (*year*; 2013--2021), survey date (*date*), survey start time (*start.time*), presence / absence of the species (*presence*; 0 [absence] or 1 [presence]), ordinal date (*date.ord*), and moon phase (%; *moon.phase*).
